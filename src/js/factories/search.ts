...
import { LR_TYPE_COURSE, LR_TYPE_VIDEO, LR_TYPE_PODCAST, LR_TYPE_PODCAST_EPISODE, OCW_PLATFORM, ocwPlatform, DATE_FORMAT, COURSE_ARCHIVED, COURSE_CURRENT, OPEN_CONTENT, PROFESSIONAL, CERTIFICATE, LR_TYPE_RESOURCEFILE, CONTENT_TYPE_VIDEO, CONTENT_TYPE_PDF, CONTENT_TYPE_PAGE } from "../lib/constants";
...
export const makeRun = () => {
  return {
    ...
    instructors: [`${casual.name} ${casual.name}`, `${casual.name} ${casual.name}`],
    prices: [{
      mode: "audit",
...
export const makeCourseResult = () => ({
  ...
  audience: casual.random_element([[], [OPEN_CONTENT], [PROFESSIONAL], [OPEN_CONTENT, PROFESSIONAL]]),
...
export const makeResourceFileResult = () => ({
  ...
  content_type: casual.random_element([CONTENT_TYPE_VIDEO, CONTENT_TYPE_PDF, CONTENT_TYPE_PAGE]),
