...
import { LR_TYPE_COURSE, LR_TYPE_RESOURCEFILE } from "@mitodl/course-search-utils/dist/constants";
...
const COURSE_FACETS = [["level", "Level", false], ["topics", "Topics", true], ["course_feature_tags", "Course Features", true], ["department_name", "Department", true]];
// TBD
...
export default function SearchPage() {
  ...
  const runSearch = useCallback(async (text, activeFacets, from) => {
    ...
    if (!emptyOrNil(suggest) && !emptyOrNil(text)) {
      setSuggestions(without([text.toLowerCase().trim().replace(/^"(.*)"$/, "$1").replace(/[\W]+/g, " ").trim()], suggest).map(suggestion => isDoubleQuoted(text) ? `"${suggestion}"` : suggestion));
    } else {
      setSuggestions(null);
    }

    setSearchFacets(new Map(Object.entries(newResults.aggregations ?? {})));
    setSearchResults(from === 0 ? newResults.hits.hits : [...results, ...newResults.hits.hits]);
    setTotal(newResults.hits.total);
    setCompletedInitialLoad(true);
  }, [setSearchResults, results, setTotal, setCompletedInitialLoad, setSuggestions]);
  ...
  const setURLParam = useCallback(debounce(newSearch => {
  ...
  const {
    ...
    clearAllFilters,
    acceptSuggestion
  } = useCourseSearch(runSearch, clearSearch, setURLParam, facets, completedInitialLoad, SEARCH_PAGE_SIZE);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const toggleResourceSearch = useCallback(toggleOn => async () => {
    const toggledFacets = [["type", LR_TYPE_RESOURCEFILE, toggleOn], ["type", LR_TYPE_COURSE, !toggleOn]];
  ...
  return <div className="search-page w-100">
      ...
      <div className="container">
        <div className="row">
          ...
          <div className="search-results col-12 col-lg-8 col-xl-8 mt-3 mt-lg-6 mx-auto px-0">
            <div className={`search-toggle ${isResourceSearch ? "nofacet" : "facet"}`}>
              {!emptyOrNil(suggestions) ? <div className="row suggestions">
                  Did you mean
                  {suggestions.map((suggestion, i) => <span className="pl-1" key={i}>
                      <a className="suggestion" onClick={e => {
                  e.preventDefault();
              ...
              <ul className="nav">
                <li className="nav-item">
                  <button className={`nav-link search-nav ${isResourceSearch ? "" : "active"}`} href="#" onClick={toggleResourceSearch(false)}>
                    Courses
                  </button>
                </li>
                <li className="nav-item">
                  <button className={`nav-link search-nav ${isResourceSearch ? "active" : ""}`} href="#" onClick={toggleResourceSearch(true)}>
                    Resources
                  </button>
                </li>
              </ul>
        ...
        <div className="row">
          {isResourceSearch ? null : <SearchFilterDrawer facetMap={facetMap} facetOptions={facetOptions} activeFacets={activeFacets} onUpdateFacets={onUpdateFacets} clearAllFilters={clearAllFilters} toggleFacet={toggleFacet} />}
          <div className="search-results col-12 col-lg-8 col-xl-8 mt-3 mx-auto px-0">
            <InfiniteScroll hasMore={from + SEARCH_PAGE_SIZE < total} loadMore={loadMore} initialLoad={false} loader={completedInitialLoad ? <Spinner key="spinner" /> : null}>
              {completedInitialLoad ? results.length === 0 ? <div className="no-results-found">
                    <span>No results found for your query</span>
                  </div> : results.map((hit, idx) => <SearchResult key={`${hit._source.title}_${idx}`} object={searchResultToLearningResource(hit._source)} searchResultLayout={SEARCH_LIST_UI} />) : <Loading />}
            </InfiniteScroll>
          </div>
        </div>
      </div>
