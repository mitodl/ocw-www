...
const mockGetResults = () =>
  times(makeCourseResult, SEARCH_PAGE_SIZE).map(result => ({
    _source: result
  }))
...
const defaultCourseFacets = { ...INITIAL_FACET_STATE, type: [LR_TYPE_COURSE] }
      ...
      expect(search.mock.calls[0]).toEqual([
        {
          ...
          activeFacets: { ...defaultCourseFacets, ...params.activeFacets }
        }
      ])
    })
  })
