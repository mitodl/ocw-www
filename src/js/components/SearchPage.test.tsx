...
const mockGetResults = () => times(makeCourseResult, SEARCH_PAGE_SIZE).map(result => ({
  _source: result
}));

let resolver;

const resolveSearch = (extraData = {}) => act(async () => {
  resolver(extraData);
});
...
const defaultCourseFacets = { ...INITIAL_FACET_STATE,
  type: [LR_TYPE_COURSE]
};
const defaultResourceFacets = { ...INITIAL_FACET_STATE,
...
describe("SearchPage component", () => {
  ...
  [{
    ...
    test(`should search at startup with ${serializeSearchParams(params)}`, async () => {
  ...
  test("the user can update the search text and submit", async () => {
    ...
    wrapper.find("input").at(0).simulate("change", {
      target: {
        value: "New Search Text"
      }
    });
    await act(async () => {
      wrapper.find("SearchBox").find("i").simulate("click");
      resolver();
    });
  ...
  test("should show suggestions if present on the search result", async () => {
    ...
    wrapper.update();
    expect(wrapper.find(".suggestions").text()).toEqual("Did you mean mathematics?");
