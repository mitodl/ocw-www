...
function FilterableSearchFacet(props) {
  ...
  return results && results.buckets && results.buckets.length === 0 ? null : <div className="facets filterable-facet pb-3">
      ...
      {showFacetList ? <>
          ...
          <div className="facet-list">
            {facets.map((facet, i) => <SearchFacetItem key={i} facet={facet} isChecked={contains(facet.key, currentlySelected || [])} onUpdate={onUpdate} labelFunction={labelFunction} name={name} />)}
          </div>
        </> : null}
    </div>;
}
