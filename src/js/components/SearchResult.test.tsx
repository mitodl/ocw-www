...
describe("SearchResult component", () => {
  ...
  it("should render the things we expect for a course", () => {
    ...
    const {
      href,
      className
    } = wrapper.find(".course-title").find("a").props();
    expect(href).toBe(object.url);
    expect(className).toBe("w-100");
    expect(wrapper.find(".subtitles").first().text()).toContain("Instructors");
    expect(wrapper.find(".subtitles").first().text()).toContain(`Prof. ${object.instructors[0]}`);
    expect(wrapper.find(".cover-image").find("img").first().prop("src")).toBe(getCoverImageUrl(object));
  ...
  it("should render the things we expect for a resource", () => {
    ...
    const wrapper = render(object);
    expect(wrapper.find(".course-title").text()).toBe(`${getContentIcon(object.content_type)}${object.content_title}`);
    expect(wrapper.find(".course-title").find("a").prop("href")).toBe(object.url);
    expect(wrapper.find(".subtitles").first().text()).toBe(`${object.coursenum} ${object.run_title}`);
    expect(wrapper.find(".subtitles").at(1).text()).toContain("Topic");
    expect(wrapper.find(".cover-image").find("img").first().prop("src")).toBe(getCoverImageUrl(object));
    expect(wrapper.find("CoverImage").exists()).toBeTruthy();
  }) //
  ;
  [(LR_TYPE_RESOURCEFILE, LR_TYPE_COURSE)].forEach(objectType => {
    it("should not render a course/resource with no url", () => {
      const object = searchResultToLearningResource(makeLearningResourceResult(objectType));
      object.url = null;
      const wrapper = render(object);
  ...
  [[], null].forEach(listValue => {
    it(`should not render div for instructors, topics if they equal ${String(listValue)}`, () => {
  ...
  it("should link to the course subjects", () => {
    ...
    wrapper.find(".topic-link").forEach((link, i) => {
      ...
      expect(href).toBe(`${SEARCH_URL}?${serializeSearchParams({
        text: undefined,
        activeFacets: {
