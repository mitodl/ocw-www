...
describe("FacetDisplay component", () => {
  ...
  const render = (props = {}) => shallow(<FacetDisplay facetMap={facetMap} facetOptions={facetOptions} activeFacets={activeFacets} onUpdateFacets={onUpdateFacets} clearAllFilters={clearAllFilters} {...props} />);

  const facetMap = [["topics", "Topics"], ["types", "Types"], ["departments", "Departments"]];
  beforeEach(() => {
    activeFacets = {};
  ...
  test("shows filters which are active", () => {
    ...
    expect(wrapper.find(".active-search-filters").find("SearchFilter").map(el => el.prop("value"))).toEqual(["Pasta", "Bread", "Starch", "World Grains and Cereals"]);
