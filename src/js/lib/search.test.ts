...
import { CONTENT_TYPE_PAGE, CONTENT_TYPE_PDF, CONTENT_TYPE_VIDEO, LR_TYPE_COURSE, LR_TYPE_RESOURCEFILE } from "./constants";
import { buildSearchQuery, buildSuggestQuery, getCoverImageUrl, getResourceUrl, getResultUrl, LEARN_SUGGEST_FIELDS, RESOURCE_QUERY_NESTED_FIELDS, RESOURCEFILE_QUERY_FIELDS, searchFields } from "./search";
import { makeLearningResourceResult } from "../factories/search";
const activeFacets = { ...INITIAL_FACET_STATE,
  type: [LR_TYPE_COURSE]
};
describe("search library", () => {
  ...
  it("should include suggest query, if text", () => {
    expect(buildSearchQuery({
      text: "text!",
      activeFacets
    }).suggest).toStrictEqual(buildSuggestQuery("text!", LEARN_SUGGEST_FIELDS));
    expect(buildSearchQuery({
      activeFacets
    }).suggest).toBeUndefined();
  });
  ...
  [true, false].forEach(hasImageSrc => {
    [CONTENT_TYPE_PAGE, CONTENT_TYPE_VIDEO, CONTENT_TYPE_PDF].forEach(contentType => {
      ...
      it(`should return correct image for result w/content type ${contentType}, image_src ${result.image_src}`, () => {
        const expectedSrc = hasImageSrc ? result.image_src : `/images/${result.content_type}_thumbnail.png`;
        expect(getCoverImageUrl(result)).toBe(expectedSrc);
      });
    });
  }) //
  ;
  [[CONTENT_TYPE_PAGE, "18-23", "mech_engineering", null, "/courses/18-23/sections/mech_engineering/"], [CONTENT_TYPE_PAGE, "18-23", "mech_engineering", "https://cdn.example.com", "/courses/18-23/sections/mech_engineering/"], [CONTENT_TYPE_PDF, "https://s3.amazonaws.com/18-23/test.pdf", "shortlink1", null, "https://s3.amazonaws.com/18-23/test.pdf"], [CONTENT_TYPE_PDF, "https://s3.amazonaws.com/18-23/test.pdf", "shortlink1", "https://cdn.example.com", "https://cdn.example.com/18-23/test.pdf"], [CONTENT_TYPE_VIDEO, "https://youtube.com/?s=2335", null, "", "https://youtube.com/?s=2335"], [CONTENT_TYPE_VIDEO, "https://youtube.com/?s=2335", null, "/coursemedia", "https://youtube.com/?s=2335"], [CONTENT_TYPE_VIDEO, "/relative/url", null, false, "/relative/url"], [CONTENT_TYPE_VIDEO, "/relative/url", null, "/coursemedia", "/relative/url"]].forEach(([contentType, url, shortUrl, cdnPrefix, expectedUrl]) => {
    it(`should return correct url for content type ${contentType} if the cdn is ${cdnPrefix ? "" : "not "}set`, () => {
  ...
  [LR_TYPE_COURSE, LR_TYPE_RESOURCEFILE].forEach(objectType => {
    it(`should return correct url for object type ${objectType}`, () => {
      ...
      const expected = isCourse ? `/courses/${result.runs[2].slug}/` : `/courses/${result.run_slug}/sections/${result.short_url}/`;
