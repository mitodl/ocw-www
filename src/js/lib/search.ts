...
import { LR_TYPE_COURSE, LR_TYPE_VIDEO, LR_TYPE_PODCAST, LR_TYPE_PODCAST_EPISODE, LR_TYPE_RESOURCEFILE, OCW_PLATFORM, CONTENT_TYPE_PAGE, CONTENT_TYPE_PDF, CONTENT_TYPE_VIDEO, CONTENT_TYPE_SEARCHABLE } from "./constants";

export const LEARN_SUGGEST_FIELDS = ["title.trigram", "short_description.trigram"];

const OBJECT_TYPE = "type";

export const RESOURCE_QUERY_NESTED_FIELDS = ["runs.year", "runs.semester", "runs.level", "runs.instructors^5", "department_name"];

export const RESOURCEFILE_QUERY_FIELDS = ["content", "title", "short_description", "department_name"];

export const LR_TYPE_ALL = [LR_TYPE_COURSE, LR_TYPE_VIDEO, LR_TYPE_PODCAST, LR_TYPE_PODCAST_EPISODE, LR_TYPE_RESOURCEFILE];

export const searchFields = (type: string): string => {
...
const COURSE_QUERY_FIELDS = ["title.english^3", "short_description.english^2", "full_description.english", "topics", "platform", "course_id", "coursenum^5", "offered_by", "department_name", "course_feature_tags"];
...
export const buildSearchQuery = ({
  ...
  for (const type of activeFacets.type) {
    ...
    const textQuery = emptyOrNil(text) ? {} : {
      ...
      }] : null].flat().filter(clause => clause !== null)
    };
    // buildFacetSubQuery
    ...
    Object.entries(facets).forEach(([key, values]) => {
      ...
      if (facetClausesForFacet.length > 0) {
        ...
        if (key === "level") {
          ...
          builder.agg("filter", key, aggr => aggr.orFilter("bool", filter).agg("nested", {
            path: "runs"
          }, "level", aggr => aggr.agg("terms", "runs.level", {
            size: 10000
          }, "level", aggr => aggr.agg("reverse_nested", null, {}, "courses"))));
        } else {
          builder.agg("filter", key, aggregation => aggregation.orFilter("bool", filter).agg("terms", key === OBJECT_TYPE ? "object_type.keyword" : key, {
            size: 10000
          }, key));
        }
      } else {
        if (key === "level") {
          ...
          builder.agg("terms", key === OBJECT_TYPE ? "object_type.keyword" : key, {
            size: 10000
          }, key);
        }
      }
...
export const buildOrQuery = (builder, searchType, textQuery, extraClauses) => {
  ...
  const contentFilter = searchType !== LR_TYPE_RESOURCEFILE ? [] : [{
    bool: {
  ...
  builder = builder.orQuery("bool", {
    filter: {
      bool: {
        ...
        }, ...contentFilter, ...extraClauses, // Add multimatch text query here to filter out non-matching results
        ...textFilter]
      }
    },
...
export const searchResultToLearningResource = result => ({
  ...
  topics: result.topics ? result.topics.map(topic => ({
    name: topic
  })) : [],
...
export const getCourseUrl = result => {
  ...
  return !emptyOrNil(publishedRuns) ? `/courses/${publishedRuns.sort((a, b) => a.best_start_date < b.best_start_date ? 1 : -1)[0].slug}/` : null;
};
export const getResourceUrl = result => {
  if (result.content_type === CONTENT_TYPE_PAGE) {
    ...
    try {
      ...
      const useCDN = originalUrl.hostname.match(/s3\.amazonaws\.com/) && cdnPrefix;
...
export const getResultUrl = result => result.object_type === LR_TYPE_COURSE ? getCourseUrl(result) : getResourceUrl(result);
